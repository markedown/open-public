...
This is the working brief for anyone building on open-public, whether a coding agent or a person. open-public is an open platform for structured political data: people, parties, roles, statements, news and polls. Two rules shape the whole codebase:
...
Private, machine-local context (deployment specifics, private datasets, in-progress plans) is kept in gitignored local files and never committed; when something there ships, its documentation moves into this file.
