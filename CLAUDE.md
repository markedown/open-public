...
- Transactional mail (verification, password reset) may be HTML, and every HTML mail carries a plain-text alternative with the same content, so a client that shows either sees the whole message. No mail loads anything from the network: no remote image, no tracking pixel, no web font, no analytics link. Anything visual is inline. A mail carries only what the message needs (a link, a short explanation) and never open or click tracking, in the message or at the provider, because a mail that reports when it was opened is surveillance of the person confirming an address.
