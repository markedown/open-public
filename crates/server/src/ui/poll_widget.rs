...
/// vote, the anonymous-voting island turns the options into a vote control.
pub fn poll_widget(poll: &Poll, viewer: Viewer, country: &str, pubkey: Option<&str>) -> Markup {
    ...
    html! {
        article id={"poll-" (poll.slug)} class="op-card p-6 sm:p-8" {
            ...
            @if can_vote && pubkey.is_some() {
                // The votable controls, driven by the anonymous-voting island: it
                // blinds a token, has it signed, and casts the ballot, all in the
                // browser. Hidden until the island enhances it; without JavaScript
                // the note explains why voting is unavailable.
                div data-poll-slug=(poll.slug) data-poll-country=(country)
                    ...
                    @match poll.kind.as_str() {
                        ...
                        "yesno" => { form data-vote { (grid_options(poll, total, true, "mt-6 grid grid-cols-2 gap-3")) } }
                        "scale" => { form data-vote { (grid_options(poll, total, true, "mt-6 flex flex-wrap gap-2")) } }
                        _ => { form data-vote { (stacked_options(poll, total, true)) } }
                    }
                    p data-vote-status hidden class="mt-3 text-sm text-red-600" {}
                }
                noscript {
                    p class="mt-6 text-sm text-ink-muted" { (i18n::t("Voting needs JavaScript enabled.")) }
                }
            } @else {
                @match poll.kind.as_str() {
                    "yesno" => (grid_options(poll, total, false, "mt-6 grid grid-cols-2 gap-3")),
                    "scale" => (grid_options(poll, total, false, "mt-6 flex flex-wrap gap-2")),
                    _ => (stacked_options(poll, total, false)),
                }
            }
...
/// One option: a submit button carrying the option id (the anonymous-voting
/// island reads the clicked one and casts the ballot) when the viewer can vote,
/// or a plain container when results are read-only. `class` styles it either way.
fn option_control(can_vote: bool, option_id: i64, class: &str, inner: Markup) -> Markup {
    html! {
        @if can_vote {
            button type="submit" name="option" value=(option_id) class={"cursor-pointer " (class)} {
                (inner)
            }
        } @else {
...
fn stacked_options(poll: &Poll, total: i64, can_vote: bool) -> Markup {
    html! {
...
/// tally bar behind.
fn grid_options(poll: &Poll, total: i64, can_vote: bool, container: &str) -> Markup {
    html! {
        div class=(container) {
            @for opt in &poll.options {
                @let pct = if total > 0 { opt.votes * 100 / total } else { 0 };
...
/// Multi-select layout: one form with a checkbox per option and a single submit,
/// so a voter casts several options at once. Driven by the anonymous-voting
/// island; shown only while the viewer can still vote.
fn multi_options(poll: &Poll, total: i64) -> Markup {
    html! {
        form data-vote class="mt-6" {
